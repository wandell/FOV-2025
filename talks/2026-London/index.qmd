...
F — Function
Neurons in the region share distinctive response properties (e.g., motion selectivity, color bias, disparity tuning).

A — Anatomy (architectonics)
The region has distinguishable microstructure (e.g., cytoarchitecture, myeloarchitecture, laminar patterns).

C — Connections
It has a characteristic pattern of inputs and outputs (afferent/efferent connectivity distinct from neighboring cortex).

T — Topography
It contains an orderly map of the visual field (retinotopy), often with clear field boundaries (e.g., meridian reversals).
...
This was an arbitrary and potentially incoherent way to define a visual area, and the value of the term visual area has been challenged over the years(@wandell2007-VisualFieldMaps, @hayden2025-rethinking-areas). At this point in time there seems to be good reason to clarify when we are describing a retinotopic map or a region identified by another specific criterion such as cellular properties or functional responses or connectivity.
