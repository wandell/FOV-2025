...
::: {#fig-ferrier-munk}
![A: Ferrier's location of visual cortex in the angular gyrus](images/ferrier-angular-gyrus.png){#ferrier-angular-gyrus width="80%"}

![B: Munk's location of visual cortex in the occipital lobe](images/munk-occipital-lobe.png){#munk-occipital-lobe width="80%"}
:::

::: {#ferrier-munk-james .callout-note}
Munk's comments on Ferrier's work were what one call fairly direct
...
::: {#fig-inouye-holmes}
![A: Inouye](images/inouye-map.png){#inouye-map width="80%"}

![B: Holmes](images/holmes-map.png){#holmes-map width="80%"}
:::
...
The burst of activity reporting visual areas was summarized in an important article by [@felleman1991-hierarchy]. One of the iconic images from that paper which summarizes a large number of 'visual areas' is shown in @fig-felleman-vanessen. The authos propose that visual areas are organized into a hierarchy with signals flowing from the bottom (retina, LGN, V1) to the top (peri-hippocampal regions). This image is still widely used in teaching and by speakers who wish to introduce visual cortex to a general audience.

![Felleman and Van Essen's hierarchical representation of areas in visual cortex.](images/felleman-vanessen.png){#fig-felleman-vanessen width="80%"}

::: {#visual-areas .callout-note}
A generation of scientists were taught David Van Essen's definition of a visual area. He used the acronym FACT to say that a region of cortex was an area based on measurements of its:
...
We also made measurements using a rotating wedge stimulus to identify the angular representation. These are the stimulus that subdivide the eccentricity map into different visual field maps.  With the large field of view available to the fMRI measurements, we could see multiple eccentricity maps distributed across much of visual cortex, but most easily in the calcarine region.

The data from the video are summarized in two pseudo-color maps showing the most effective visual field eccentricity for each cortical location 

::: {#fig-inouye-holmes}

![A: Eccentricity map shows the most effective visual eccentricity (deg) for each location.](images/Slide23.png){#fig-eccentricity-map width ="80%"}

![B: Angle map shows the most effective wedge angle for each location.](images/Slide24.png){#fig-angle-map width="80%"}

![C: V1-V3 shown on calcarine along with an eccentricity map emphasizing the cortical magnification.](images/Slide25.png){#fig-areas-magnification width="80%"}
...
Over time, it become clear to us that this pattern was found repeatedly.  There were separate eccentricity maps throughout much of posterior cortex. 

![Visual field map clusters in human cortex @wandell2005-visualFieldMap-clusters](images/clusters-flat.mp4){#fig-clusters-flatmap width="80%"}

Video bringing on the different clusters over time.

![The visual field map clusters, as described in @wandell2005-visualFieldMap-clusters](images/clusters.mp4){#clusters-movie width="80%" loop="true" autoplay="true" muted="true"}
...
. Furthermore, the human visual cortex contains a much larger number of neurons, which is thought to reflect the increased processing required for human-specific cognitive demands, such as language and reading
.
...
Anatomical Location of V1: In macaques, a large portion of the primary visual cortex (V1) is located on the operculum, extending posterior and lateral to the medial position of the calcarine sulcus
. In humans, V1 is located largely within the calcarine sulcus; while it sometimes extends onto the lateral surface of the occipital pole, this lateral extension is not as large or typical as it is in the macaque
.
Derived from: "Imaging retinotopic maps in the human brain" (Wandell & Winawer, 2011).
...
. In humans, V3 is proportionately much larger, meaning the scale factor difference between human and monkey V3 is even greater than the difference between their V1 maps
. This difference in the V3/V1 surface area ratio likely reflects a functional divergence between the visual systems of the two species
.
...
. In humans, investigators have not reliably found a dorsal component for the fourth visual map; instead, the human map (hV4) and the regions critical for color vision (and those associated with cerebral achromatopsia) are confined almost entirely to the ventral surface
.
Derived from: "Mapping hV4 and ventral occipital cortex: The venous eclipse" (Winawer et al., 2010), "Imaging retinotopic maps in the human brain" (Wandell & Winawer, 2011), and "Visual Field Maps in Human Cortex" (Wandell, Dumoulin, & Brewer, 2007).
-->
...
Most scientists believed that the retinotopic maps and'visual areas discovered in non-human primates would be a good model for human visual cortex. But even as late as 1991, Horton and Hoyt observed that there was no proof of even two maps maps in human visual cortex [@horton1991-quadraticdefects; @horton1991-representationvisualfield]. The need for guessing changed with the introduction of functional magnetic resonance imaging (fMRI). That technology enabled investigators to identify retinotopic maps in the living human brain at the resolution of a few milimeters. Over the years, the spatial precision of the mapping and the efficiency of the measurements both increased dramatically. This technique replaced the "dismaying exercise in tedium" required to identify maps using microelectrodes [@hubel1977].

Initially, Steve Engel and I - with many others - developed a technique to measure eccentricity and angle maps using rings and wedges. An example of the stimulus and fMRI response - sped up in time - is shown in @fig-eccentricity-ring. If you look at the video casually, you will see the main wave from the occipital pole traveling in the anterior direction of the main map. If you look a little more closely, you will also see a second traveling wave on the ventral occippital-temporal (VOT) surface that starts when the stimulus is in the fovea and then fades out as the ring expands about half way (10 deg). The VOT eccentricity map is a second eccentricity map, confined mainly to the perifoveal visual field, that is distinct from the calcarine map.

![Eccentricity mapping using an expanding ring stimulus (20 deg) in the calcarine and ventral occipital region](../../chapters/videos/eccentricity-map-encyclopedia.mp4){#fig-eccentricity-ring width="80%" loop="true" autoplay="true" muted="true"}

The existence of multiple maps is shown by using a second stimulus, a rotating wage.  That stimulus measures the most effective stimulus angle, rather than eccentricity.



With the large field of view available to the fMRI measurements, we could see multiple eccentricity maps distributed across the
>>>>>>> Stashed changes
